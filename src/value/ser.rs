...
mod test {
    ...
    fn test_struct() -> anyhow::Result<()> {
        ...
        let config = Value::try_serialize(&test)?;
        let actual: Test = config.try_deserialize()?;
        assert_eq!(test, actual);
        Ok(())
    }

    #[test]
    fn test_nest() -> anyhow::Result<()> {
        ...
        let config = Value::try_serialize(&val)?;
        let output: serde_json::Value = config.try_deserialize()?;
